...
ds = pd.read_csv("aann_corruption_pos.csv")
...
for index, row in tqdm(ds.iterrows(), total=ds.shape[0]):
    prefix = row["prefixes_pos"]
    slor_correct = compute_slor(prefix, row["construction_pos"])
    slor_order_swap = compute_slor(prefix, row["order_swap_pos"])
    slor_no_article = compute_slor(prefix, row["no_article_pos"])
    slor_no_modifier = compute_slor(prefix, row["no_modifier_pos"])
    slor_no_numeral = compute_slor(prefix, row["no_numeral_pos"])
